      ...
      <div class="signup-card">
        ...
        <form @submit.prevent="signUp">
          ...
          <div class="input-group">
            ...
            <input type="text" id="name" v-model="name" placeholder="홍길동" required autocomplete="name" />
          </div>

          <div class="input-group">
            <label for="identifier">이메일 (아이디)</label>
            <input type="email" id="identifier" v-model="identifier" placeholder="example@email.com" required autocomplete="username"/>
          </div>
          ...
          <div v-if="userType === 'sitter'" class="conditional-section">
            ...
            <div class="input-group">
              ...
              <div class="address-group">
                ...
                <button type="button" class="address-btn" @click="addTeacherRegion" :disabled="!teacherRegionSelect.district">추가</button>
              </div>
              ...
              <div class="selected-tags-area" v-if="teacherInfo.selectedRegions.length > 0">
                <span v-for="region in teacherInfo.selectedRegions" :key="region" class="tag">
                  {{ region }} <button type="button" @click="removeTeacherRegion(region)">x</button>
                </span>
              </div>
            </div>
          ...
          <BaseButton type="submit" :disabled="!agreed || isSubmitting">
    ...
    async mockReverseGeocoding(lat, lon) {
      ...
      try {
        const response = await axios.get('https://nominatim.openstreetmap.org/reverse', {
          params: { format: 'json', lat: lat, lon: lon, zoom: 18, addressdetails: 1, 'accept-language': 'ko' }
        });
    ...
    async signUp() {
      ...
      try {
        ...
        if (this.userType === 'parent') {
          ...
          // sitter의 경우 리스트로 보냅니다.
          regionValue = this.teacherInfo.selectedRegions;
        }

        // [Fix 422]: null 대신 빈 배열([])을 기본값으로 사용하여 Type Error 방지
        ...
        const formData = {
          ...
          // [Fix 422]: null 전송 방지 (서버 스키마에 따라 빈 리스트 혹은 기본값 전송)
          children_profiles: childrenProfiles,
          
          warning: this.userType === 'parent' ? this.parentInfo.notes : null,
          
          // [Fix 422]: 리스트 타입을 기대하는 필드에 null 대신 [] 전송
          pay_period: this.userType === 'sitter' ? this.teacherInfo.paymentCycles : [],
          
          cctv_agree: this.userType === 'sitter' ? (this.teacherInfo.cctvAgree === 'agree') : false,
          ...
          certifications: this.userType === 'sitter' ? this.teacherInfo.certifications.filter(c => c.trim() !== '') : [],
        ...
        if (error.response) {
          ...
          if (error.response.status === 500) {
            ...
            if (Array.isArray(errorDetail)) {
              ...
              alertMsg = errorDetail || '회원가입 중 오류가 발생했습니다. (422 Error)';
            }
            alert('가입 실패:\n' + alertMsg);
          }
        } else {
...
.counter-box { display: flex; align-items: center; justify-content: space-between; border: 1px solid #e0e0e0; border-radius: 10px; width: 160px; overflow: hidden; background-color: white; }
.counter-btn { background-color: #f8f9fa; border: none; padding: 12px 18px; font-size: 18px; color: #555; cursor: pointer; transition: background-color 0.2s; display: flex; align-items: center; justify-content: center; }
.counter-btn:hover:not(:disabled) { background-color: #e9ecef; }
.counter-btn:disabled { color: #ccc; cursor: not-allowed; background-color: #f1f3f5; }
.count-display { font-size: 16px; font-weight: 700; color: #333; flex: 1; text-align: center; user-select: none; }
.location-btn { background-color: #ffffff; border: 1px solid #4CAF50; color: #4CAF50; padding: 8px 16px; border-radius: 8px; cursor: pointer; font-size: 13px; font-weight: bold; transition: all 0.2s; width: 100%; }
.location-btn:hover { background-color: #e8f5e9; }
.location-btn:disabled { border-color: #ccc; color: #999; cursor: not-allowed; background-color: #f9f9f9; }
.status-msg { font-size: 13px; margin-top: 5px; font-weight: bold; }
